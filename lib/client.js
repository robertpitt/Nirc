...
{
	console.log("C: %s (" + message + ")", this.mask())
}
...
Client.prototype.register = function()
{
	if(this.registered === false && this.nick && this.username)
	{
		 ...
		 */
		this.send(this.mask(), '001', this.id(), 'Welcome to tje INternet Relay Network', this.mask())
		this.send(this.mask(), '002', this.id(), 'Your host is', "HOST", 'running version', '1.0.0')
		this.send(this.mask(), '003', this.id(), 'This server was created on', "@DATA_HERE@");
		this.send(this.mask(), '004', this.id(), "@NAME@", "@VERSION@", "@USERMODES@", "@CHANMODES@");

		/**
		 * Send MOTD
		 */
		this.send(this.mask(), '375', this.id(), ":-","@SERVER@", "of the Day -");
